...
export async function generateEmbeddingWithCache(
  ...
  if (type === 'query') {
    ...
    console.log('❌ Query embedding cache MISS - generating new embedding');
  }
...
export async function generateBatchEmbeddingsOptimized(
  ...
  if (duplicateCount > 0) {
    console.log(`♻️  Detected ${duplicateCount} duplicate chunks - will reuse embeddings`);
  }
  ...
  for (const text of uniqueTexts) {
    ...
    if (cached) {
      ...
      console.log(`✅ Content hash cache HIT for hash ${hash.substring(0, 8)}...`);
    } else {
  ...
  if (cacheHitCount > 0) {
    console.log(`💰 Saved ${cacheHitCount} embedding API calls via content deduplication`);
  }

  // Generate embeddings for new content
  let newEmbeddings: number[][] = [];
  if (textsToEmbed.length > 0) {
    console.log(`🔄 Generating embeddings for ${textsToEmbed.length} unique new texts`);
    ...
    try {
      ...
      console.log(`💾 Cached ${textsToEmbed.length} new content embeddings`);
    } catch (error) {
  ...
  console.log(`✅ Batch complete: ${textsToEmbed.length} API calls, ${apiCallsSaved} saved`);
...
export async function findSimilarCachedQuery(
  ...
  if (exactMatch) {
    console.log('✅ Exact query match found in cache');
...
export function clearAllCaches() {
  ...
  console.log(`🗑️  Cleared ${queryCount} query cache entries and ${contentCount} content cache entries`);
}
