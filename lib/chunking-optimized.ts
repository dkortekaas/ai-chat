...
export interface OptimizedChunkOptions {
  chunkSize: number;
  chunkOverlap: number;
...
export function chunkTextOptimized(
  ...
  if (cleanText.length < minChunkSize) {
    console.log(`⚠️  Text too small (${cleanText.length} chars), skipping`);
    return [];
  }
  ...
  while (startIndex < cleanText.length) {
    ...
    if (chunkContent.length >= minChunkSize) {
      ...
      console.log(`⚠️  Skipping small chunk (${chunkContent.length} chars)`);
    }
  ...
  console.log(`✅ Created ${chunks.length} optimized chunks (avg: ${Math.round(cleanText.length / chunks.length)} chars/chunk)`);
...
export function chunkWebsiteContentOptimized(
  ...
  console.log(`✅ Created ${chunks.length} optimized website chunks`);
...
export function compareChunkingStrategies(text: string) {
  ...
  console.log(`Old: ${oldStats.totalChunks} chunks, ${oldStats.totalTokens} tokens`);
  console.log(`New: ${newStats.totalChunks} chunks, ${newStats.totalTokens} tokens`);
  console.log(`Reduction: ${chunkReduction.toFixed(1)}% fewer chunks = ${costReduction.toFixed(1)}% cost savings`);
