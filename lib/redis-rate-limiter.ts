...
try {
  if (process.env.UPSTASH_REDIS_REST_URL && process.env.UPSTASH_REDIS_REST_TOKEN) {
    ...
    console.log("✅ Upstash Redis rate limiter initialized");
  } else {
    console.warn(
...
export async function checkRateLimit(
  ...
  try {
    ...
    if (!result.success) {
      console.warn(
        `⚠️ Rate limit exceeded for key: ${key.substring(0, 12)}... (${limit - result.remaining}/${limit} requests)`
      );
    }
...
export async function resetRateLimit(key: string): Promise<void> {
  ...
  try {
    ...
    console.log(`Rate limit reset requested for ${key} - entries will expire automatically`);
  } catch (error) {
